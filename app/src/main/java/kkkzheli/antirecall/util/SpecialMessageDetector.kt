...
class SpecialMessageDetector {
    ...
    fun getSpecialDisplayText(
        ...
        return when (specialType) {
            SpecialType.VOICE_CALL -> "Speech call\ntitle: ${title.ifEmpty { "语音通话" }}"
            SpecialType.VIDEO_CALL -> "Video call\n${if (title.isNotEmpty()) "$title\n" else ""}$text"
            SpecialType.TRANSFER -> "Transfer notification\n$text"
            SpecialType.RED_PACKET -> "Red packet received\n$text"
        }
    }
}
